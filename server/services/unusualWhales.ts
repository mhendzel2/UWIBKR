...
export class UnusualWhalesService {
  ...
  async getSectorPerformance(): Promise<any[]> {
    try {
        ...
        // Fetch sector performance data from the API
        const response = await this.makeRequest<{ data: any[] }>(`/market/sector-performance`);
        const sectorData = response.data || [];

        // Process the data to match the app's requirements
        const sectorPerformance = sectorData.map((sector: any) => {
            ...
            return {
                ...
                sentiment,
                topTickers: sector.tickers.slice(0, 3),
  ...
  async getMarketOverview(): Promise<any> {
    try {
        ...
        const [marketTide, totalVolumeResponse, spyData, flowAlerts] = await Promise.all([
            this.makeRequest<{ data: any[] }>(`/market/market-tide`).catch(() => null),
            this.makeRequest<{ data: { put_volume: string; call_volume: string; total_volume: string } }>(`/market/total-options-volume`).catch(() => null),
            this.makeRequest<{ data: any }>(`/stock/SPY/net-prem-ticks`).catch(() => null),
            this.getFlowAlerts({ minPremium: 1000000 }).catch(() => [])
        ]);

        const totalVolume = totalVolumeResponse?.data || { put_volume: '0', call_volume: '0', total_volume: '0' };
        ...
        if (totalVolume.put_volume && totalVolume.call_volume) {
        ...
        const marketOverview = {
            ...
            optionsMetrics: {
                totalVolume: parseFloat(totalVolume.total_volume),
